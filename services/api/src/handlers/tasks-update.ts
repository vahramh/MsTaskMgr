...
import type { UpdateTaskRequest, UpdateTaskResponse, TaskStatus, WorkflowState, EntityType } from "@tm/shared";
...
export const handler = withHttp(
  async (event: APIGatewayProxyEventV2, ctx: HttpHandlerContext): Promise<APIGatewayProxyResultV2> => {
    ...
    if (body.status !== undefined) {
      ...
      if ((body as any).state === undefined) {
        patch.state = body.status === "COMPLETED" ? "completed" : "inbox";
      }
    }
    ...
    try {
      ...
      // If this is a v1 item, persist v2 defaults as part of this update.
      ...
      // Guard: reopen is not allowed via generic update. Must use a dedicated endpoint later.
      ...
      log("error", "tasks.update_failed", { requestId, sub, taskId, error: toErrorInfo(e) });
      return internalError("Failed to update task", undefined, requestId);
    }
  }
);
